  ...
  implements ReasonPhraseCatalog
{
  ...
  public String getReason(int paramInt, Locale paramLocale)
  {
    ...
    paramLocale.append("Unknown category for status code ");
    paramLocale.append(paramInt);
    Args.check(bool, paramLocale.toString());
    int i = paramInt / 100;
    paramInt -= i * 100;
    paramLocale = REASON_PHRASES;
    if (paramLocale[i].length > paramInt) {
      return paramLocale[i][paramInt];
    }
    return null;
  }
}
